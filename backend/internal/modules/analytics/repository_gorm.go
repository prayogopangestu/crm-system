...
func (r *gormRepository) DashboardStats(ctx context.Context, organizationID string, now time.Time) (DashboardStats, error) {
	...
	stats.TotalRevenue = postgresx.FormatRupiah(revenue)
	stats.RevenueTrend = "Stabil"
	return stats, nil
}

func (r *gormRepository) ConversionChart(ctx context.Context, organizationID string, now time.Time) ([]ConversionPoint, error) {
	...
	defer rows.Close()
	items := make([]ConversionPoint, 0, 6)
	for rows.Next() {
		var item ConversionPoint
...
func (r *gormRepository) Activities(ctx context.Context, organizationID string, limit int) ([]Activity, error) {
	rows, err := r.store.Query(ctx).Raw(`
...
func (r *gormRepository) Leaderboard(ctx context.Context, organizationID string, month time.Time) ([]LeaderboardEntry, error) {
...
func (r *gormRepository) LostReasons(ctx context.Context, organizationID string) ([]LostReason, error) {
	rows, err := r.store.Query(ctx).Raw(`
...
func (r *gormRepository) Goals(ctx context.Context, organizationID string) ([]PerformanceGoal, error) {
	rows, err := r.store.Query(ctx).Raw(`
	...
	defer rows.Close()
	items := make([]PerformanceGoal, 0)
	for rows.Next() {
		var month time.Time
		var item PerformanceGoal
